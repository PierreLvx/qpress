...
void *decompress_file_thread(void *arg)
{
    ...
    for(;;)
    {
        ...
        // decompressed-size bound is a separate, independently attacker-controlled
        // header field - without it, QLZ_DECOMPRESS() below would trust it as the
        // write bound and happily overflow dst[thread_id], which only has room for
        ...
        if (QLZ_SIZE_COMPRESSED(src[thread_id]) < 9 || QLZ_SIZE_COMPRESSED(src[thread_id]) > compress_chunk_size + QLZ_SIZE_OVERHEAD ||
            QLZ_SIZE_DECOMPRESSED(src[thread_id]) > compress_chunk_size + QLZ_SIZE_OVERHEAD)
            abort("Data error, not recoverable"); // todo, it is recoverable, but code is not implemented yet
